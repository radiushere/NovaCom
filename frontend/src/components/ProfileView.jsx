...
const ProfileView = ({ targetId, currentUserId, returnPath, onBack }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fade-in pb-10 relative">
      ...
      {/* NEW: Back Button (Only shows if there is a return path) */}
      {returnPath && (
          <button 
            onClick={onBack}
      ...
      <GlassCard className="relative overflow-hidden p-0 mt-8"> {/* Added margin top for button */}
            ...
            <div className="mt-8 text-center space-y-8 w-full max-w-2xl px-4">
                <div className="flex flex-wrap justify-center gap-2">
                    {user.tags && user.tags.map(t => (
                        <span key={t} className="px-4 py-1.5 bg-cyan-supernova/5 text-cyan-supernova border border-cyan-supernova/20 rounded-full text-sm font-medium">#{t}</span>
                    ))}
                </div>
                <div className="grid grid-cols-2 gap-4 text-left">
                    <div className="bg-white/5 p-6 rounded-2xl border border-white/5 hover:border-white/10 transition">
                        ...
                        <p className="text-white font-mono text-sm truncate">{isSelf ? user.email : "Hidden (Privacy Protocol)"}</p>
                    </div>
                    <div className="bg-white/5 p-6 rounded-2xl border border-white/5 hover:border-white/10 transition">
                ...
                <div className="pt-4">
                    {isSelf ? (
                        <button onClick={() => setIsEditing(true)} className="bg-white/10 hover:bg-white/20 text-white px-10 py-3 rounded-xl border border-white/20 transition font-bold tracking-wide">EDIT PROFILE</button>
                    ) : isFriend ? (
                        <button disabled className="bg-gray-800 text-gray-400 font-bold px-10 py-3 rounded-xl cursor-not-allowed border border-white/5 flex items-center gap-2 mx-auto"><span>✅</span> Signal Connected</button>
                    ) : (
                        <button onClick={handleConnect} className="bg-cosmic-purple hover:bg-purple-500 text-white font-bold px-10 py-3 rounded-xl shadow-lg shadow-purple-500/20 transition transform hover:scale-105">CONNECT SIGNAL</button>
                    )}
                </div>
            </div>
          )}
